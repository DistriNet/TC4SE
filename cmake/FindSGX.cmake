...
# find_path(SGXSSL_INCLUDE_DIR sgx_tsgxssl.edl "${SGXSSL_INCLUDE_PATH}" NO_DEFAULT_PATH)
...
if(SGX_FOUND)
    ...
    if(SGX_HW)
        ...
        set(SGX_DCAP_LIB sgx_dcap_ql)
        set(SGX_UAE_LIB sgx_quote_ex)
    else()
        ...
        set(SGX_DCAP_LIB sgx_dcap_ql_sim)
        set(SGX_UAE_LIB sgx_quote_ex_sim)
    endif()
    ...
    function(_build_edl_obj edl edl_search_paths use_prefix header_only)
        ...
        if(NOT ${header_only})
            ...
            add_custom_command(OUTPUT ${EDL_T_H}
                COMMAND ${SGX_EDGER8R} ${USE_PREFIX} --trusted ${EDL_ABSPATH} --search-path ${SEARCH_PATHS} --header-only
                MAIN_DEPENDENCY ${EDL_ABSPATH}
                WORKING_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR})
            add_custom_target(${target}-edl-header DEPENDS ${EDL_T_H})

            add_library(${target}-edlobj OBJECT ${CMAKE_SOURCE_DIR}/cmake/Blank.cpp)
            add_dependencies(${target}-edlobj ${target}-edl-header)
            target_include_directories(${target}-edlobj INTERFACE ${CMAKE_CURRENT_BINARY_DIR} ${ENCLAVE_INC_DIRS})
            target_compile_options(${target}-edlobj INTERFACE -include${EDL_T_H})
        endif()
    ...
    function(add_trusted_library target)
        ...
        if("${SGX_EDL}" STREQUAL "")
            ...
            add_library(${target} OBJECT ${SGX_SRCS})
        else()
            if("${SGX_EDL_SEARCH_PATHS}" STREQUAL "")
                message("${target}: SGX enclave edl file search paths are not provided!")
            endif()
            if(${SGX_USE_SGXSSL})
                set(SGX_EDL_SEARCH_PATHS "${SGX_EDL_SEARCH_PATHS};${SGXSSL_INCLUDE_PATH}")
            endif()
            _build_edl_obj(${SGX_EDL} "${SGX_EDL_SEARCH_PATHS}" ${SGX_USE_PREFIX} TRUE)
            add_library(${target} OBJECT ${SGX_SRCS} $<TARGET_OBJECTS:${target}-edlobj>)
            target_link_libraries(${target} PRIVATE ${target}-edlobj)
        endif()
        ...
        target_compile_options(${target} PRIVATE -include${SGX_LIBCXX_INCLUDE_DIR}/__sgx)
        ...
        if(${SGX_USE_SGXSSL})
            ...
            #     -Wl,--start-group  -lsgx_dcap_tvl -lsgx_tsgxssl -lsgx_tsgxssl_crypto -lsgx_tsgxssl_ssl -lsgx_tstdc -lsgx_pthread -lsgx_tcxx -lsgx_tkey_exchange -lsgx_tcrypto -l${SGX_TSVC_LIB} -Wl,--end-group \
            ...
            #     -Wl,--start-group -lsgx_tstdc -lsgx_dcap_tvl -lsgx_tcxx -lsgx_tkey_exchange -lsgx_tcrypto -l${SGX_TSVC_LIB} -Wl,--end-group \
            #     -Wl,-Bstatic -Wl,-Bsymbolic -Wl,--no-undefined \
            #     -Wl,-pie,-eenclave_entry -Wl,--export-dynamic \
            #     ${LDSCRIPT_FLAG} \
            #     -Wl,--defsym,__ImageBase=0")
        ...
        target_compile_definitions(${target} PRIVATE -DTRUSTED_BUILD)
    endfunction()
    ...
    function(add_enclave_library target)
        set(optionArgs USE_PREFIX USE_SGXSSL)
        set(oneValueArgs EDL LDSCRIPT)
        set(multiValueArgs SRCS TRUSTED_LIBS EDL_SEARCH_PATHS LINK_WITH)
        cmake_parse_arguments("SGX" "${optionArgs}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})
        if("${SGX_EDL}" STREQUAL "")
        ...
        _build_edl_obj(${SGX_EDL} "${SGX_EDL_SEARCH_PATHS}" ${SGX_USE_PREFIX} FALSE)
        ...
        foreach(TLIB ${SGX_LINK_WITH})
            target_link_libraries(${target} PRIVATE ${TLIB})
            add_dependencies(${target} ${TLIB})
        endforeach()
        ...
        if(${SGX_USE_SGXSSL})
            ...
            target_link_libraries(${target} PRIVATE "${SGX_COMMON_CFLAGS} \
                -Wl,--no-undefined -nostdlib -nodefaultlibs -nostartfiles -L${SGX_LIBRARY_PATH} -L${SGXSSL_LIBRARY_PATH} \
                -Wl,--whole-archive -l${SGX_TRTS_LIB} -lsgx_dcap_tvl -lsgx_tsgxssl -lsgx_ttls -Wl,--no-whole-archive \
                -Wl,--start-group ${TLIB_LIST} -lsgx_tsgxssl_crypto -lsgx_tsgxssl_ssl -lsgx_tstdc -lsgx_pthread -lsgx_tcxx -lsgx_tkey_exchange -lsgx_tcrypto -l${SGX_TSVC_LIB} -Wl,--end-group \
                -Wl,-Bstatic -Wl,-Bsymbolic -Wl,--no-undefined \
                -Wl,-pie,-eenclave_entry -Wl,--export-dynamic \
                ${LDSCRIPT_FLAG} \
                -Wl,--defsym,__ImageBase=0")
            message("TARGET LINK LIBRARY 1: ${target}")
        else()
            target_link_libraries(${target} PRIVATE "${SGX_COMMON_CFLAGS} \
                -Wl,--no-undefined -nostdlib -nodefaultlibs -nostartfiles -L${SGX_LIBRARY_PATH} \
                -Wl,--whole-archive -l${SGX_TRTS_LIB} -Wl,--no-whole-archive \
                -Wl,--start-group ${TLIB_LIST} -lsgx_dcap_tvl -lsgx_tstdc -lsgx_tcxx -lsgx_ttls -lsgx_tkey_exchange -lsgx_tcrypto -l${SGX_TSVC_LIB} -Wl,--end-group \
                -Wl,-Bstatic -Wl,-Bsymbolic -Wl,--no-undefined \
                -Wl,-pie,-eenclave_entry -Wl,--export-dynamic \
                ${LDSCRIPT_FLAG} \
                -Wl,--defsym,__ImageBase=0")
            message("TARGET LINK LIBRARY 2: ${target}")
        endif()
    ...
    function(add_untrusted_library target mode)
        ...
        foreach(EDL_HDR ${EDL_U_HDRS})
            target_compile_options(${target} PRIVATE -include${EDL_HDR})
        endforeach()
